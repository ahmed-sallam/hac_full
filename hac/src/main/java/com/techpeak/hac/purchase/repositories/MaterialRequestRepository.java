...
public interface MaterialRequestRepository extends JpaRepository<MaterialRequest, Long>, JpaSpecificationExecutor<MaterialRequest> {
    ...
    @Query(value = "WITH material_request_info AS ( " +
            ...
            "         COALESCE(i2.quantity, 0) AS store_quantity, " +
            ...
            "  GROUP BY mrl.quantity, mrl.notes, mrl.material_request_id, p.id, p.product_number, " +
            ...
            "       )) FILTER (WHERE mrl.material_request_id IS NOT NULL), '[]') AS lines, " +
            "       COALESCE(json_agg(DISTINCT jsonb_build_object( " +
    ...
    MaterialRequest updateStatusById(@Param("id") Long id, @Param("status") String status);
