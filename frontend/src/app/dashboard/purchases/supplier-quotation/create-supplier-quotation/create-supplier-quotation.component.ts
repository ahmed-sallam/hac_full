...
export class CreateSupplierQuotationComponent implements OnInit {
  ...
  FormArray = FormArray;
  supplierQuotationService: SupplierQuotationService = inject(
  ...
  createExpensesLine(): FormGroup {
    return this.fb.group({
      expensesTitleId: new FormControl("", [
        Validators.required,
  ...
  getExpensesTitles(s: string) {
    this.purchaseExpensesTitle$ = this.purchaseService.getExpensesTitles().pipe(
      map((res: PurchaseExpensesTitle[]) => {
        return res;
      }),
      catchError((err: any) => {
        return [];
  ...
  searchSuppliers(s: string) {
    this.suppliers$ = this.suppliersService.getSubbliers(0, 80, s.trim()).pipe(
      map((res: SuppliersResponse) => {
        return res.content;
      }),
      catchError((err: any) => {
        return [];
  ...
  addSupplierQuotation(data: CreateSupplierQuotation) {
    this.supplierQuotationService.addSupplierQuotation(data).subscribe(
      (res) => {
      ...
      next: (res) => {
        res.lines.forEach((line: any) => {
          const v =
            ...
            " " +
            (true
              ? line.product.mainBrandAr + " - " + line.product.subBrandAr
              : line.product.mainBrandEn + " - " + line.product.subBrandEn);
          this.lines.push(
            this.fb.group({
              ...
              productId: new FormControl(line.product.id, [
                Validators.required,
                Validators.pattern(/^(\s+\S+\s*)*(?!\s).*$/),
              ]),
